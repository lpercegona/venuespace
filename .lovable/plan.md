...
- `previewBookingQuote({ category_id, layout_draft })`: gera PDF de amostra a partir da configuração ainda não salva, usando um registro real de reserva quando existir, senão dados fictícios coerentes por tipo de campo. Não persiste nada.
- Rodapé de assinatura fixo (não editável), desenhado no rodapé de **todas** as páginas: à esquerda "Orçamento gerado através da plataforma venuespace.", ao centro a logo do Venuespace, à direita "venuespace.com.br" como link clicável para https://venuespace.com.br (anotação de link `pdf-lib`).
...
**Frontend**
- Em `/admin` → categoria → nova aba "Modelo de Orçamento (PDF)": lista dos campos disponíveis na tabela-modelo de reservas da categoria + botão "adicionar ao modelo".
...
- Auditoria de cobertura: varrer os três escopos (organização, tabela, registro) e garantir que "Todos os campos" liste realmente todos, incluindo os que hoje ficam de fora, todos editáveis, mantendo o padrão estrutural atual.
