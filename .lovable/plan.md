...
Plano validado e revisado com a **Diretriz de Desenvolvimento** promovida a princípio soberano do projeto — ela precede e condiciona todas as demais seções, inclusive o Design System. Correções críticas mantidas (public_form com tabela de destino separada + relação automática; motor de reserva com verificação de conflito de datas). Núcleo desenhado para evoluir para Business Operating System (módulos, construtores visuais, White Label, domínios próprios, marketplace, SDK) sem reestruturar o core.

Ao aprovar, materializarei este documento como skill ativa em `.agents/skills/venuespace/SKILL.md` (frontmatter `name: venuespace` + descrição gatilho cobrindo: motor de dados multi-tenant, tabelas/campos dinâmicos, publicação pública seletiva, formulários públicos com captação de leads, chat com proposta comercial, campanhas de arrecadação com confirmação manual, motor de reserva com conflito de datas, Design System shadcn/ui + Tailwind v4 e Diretriz de Desenvolvimento estrita) e a aplicarei via `skills--apply_draft`.

---

## 0. Diretriz de Desenvolvimento (soberana, obrigatória)

Vale acima de qualquer outra seção deste documento. Em caso de conflito entre esta diretriz e qualquer decisão de design, arquitetura, escopo ou implementação sugerida em iteração, **prevalece esta diretriz**.

- **Estritamente técnico e objetivo**: implementar exatamente o que está documentado neste plano. Sem interpretações subjetivas, simplificações "de bom senso", mudanças de comportamento, decisões de produto ou alterações de arquitetura não especificadas.
...
- **Validação técnica obrigatória antes de fechar etapa**: verificar build, verificar rotas afetadas em light+dark e mobile+desktop, verificar que iterações anteriores continuam funcionando (sem regressão), verificar RLS e permissões nos endpoints tocados.
- **Sem "melhorias" implícitas**: refactors, renomeações e otimizações fora do escopo só ocorrem por solicitação explícita.

Todo o restante deste documento é lido sob essa lente.
...
MVP apenas. Arquitetura preparada para evoluir a Business Operating System (módulos, construtores visuais, White Label, domínios próprios, marketplace, SDK) sem reestruturar o núcleo.
...
- Bordas: default de v4 é `currentColor`; usar sempre `border border-border` (ou variante semântica) — reset base atual já faz `* { border-color: var(--color-border) }`, manter.
...
- **Ícones**: apenas `lucide-react` (alinhado com `components.json`).
...
- **Feedback**: `sonner` para toast; `AlertDialog` para confirmação destrutiva; `Skeleton` para loading; `EmptyState` padrão para vazio; boundary de erro por rota (padrão TanStack) já obrigatório.
...
- Radius consistente via `--radius` (base) e utilities `rounded-sm/md/lg/xl/2xl` (já registrados em `@theme inline`); proibido `rounded-[Xpx]` arbitrário.
