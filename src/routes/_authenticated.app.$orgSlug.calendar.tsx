...
function CalendarPage() {
  ...
  return (
    <AppShell title="Calendário de ocupação" subtitle="Reservas aceitas ou fechadas por tabela reservável.">
      {tables.isLoading ? (
        <div className="flex justify-center py-10"><Loader2 className="h-5 w-5 animate-spin text-muted-foreground" /></div>
      ) : (
        <div className="space-y-6">
          {(tables.data ?? []).filter((t: any) => t.bookable).length === 0 ? (
            <EmptyState title="Nenhuma tabela reservável" description="Marque uma tabela como 'com reservas' para ver ocupação aqui." />
          ) : null}
    ...
    <Card>
      ...
      <CardContent>
        {q.isLoading ? (
          ...
          <p className="text-sm text-muted-foreground">Sem reservas confirmadas.</p>
        ) : (
