...
- **Correções não abrem iteração**: quando a entrega corrige comportamento de escopo já entregue (bug, regressão, plotagem incorreta, dado corrompido por bug anterior), ela **não recebe novo número de iteração**. É registrada como `Correção`, identificando explicitamente as iterações corrigidas — `Correção da Iteração N` ou `Correção das Iterações N/M` — no título da entrada do `CHANGELOG.md`. A entrada deve citar a iteração de origem do bug, a causa raiz verificada e a auditoria de propagação correspondente. Nova numeração de iteração é reservada exclusivamente a escopo novo aprovado pelo usuário. Antes de numerar qualquer entrega, verificar as iterações anteriores no `CHANGELOG.md` e, se o escopo já existir, tratar como correção.
