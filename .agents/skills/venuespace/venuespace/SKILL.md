...
Antes de escrever qualquer código, listar explicitamente toda superfície existente que lê, escreve ou exibe a entidade/tabela/campo afetado pela mudança — telas internas (todas as roles: super_admin, owner, editor, viewer), endpoints públicos (/api/public/*), buscas/filtros, campos computados que dependem do dado, navegação/menus, e formulários de criação/edição relacionados. Se a mudança introduz um novo tipo de campo, tipo de tabela ou papel, verificar seu efeito em: DynamicGrid, DynamicForm, resolução de computed, exportação (se existir), e qualquer endpoint público que hoje devolve a entidade.  
Auditoria de propagação obrigatória (antes de fechar a iteração):  
Para cada item do mapeamento acima, declarar explicitamente: (a) foi atualizado nesta iteração, ou (b) não precisava ser tocado, com justificativa técnica de por quê é seguro deixá-lo como está. Não é permitido declarar a iteração concluída sem essa lista publicada.
...
super admin (administração total de padrões da instancia,owner (total), editor (records + negociação + confirmação de contribuição), viewer (leitura), autenticado não-membro (submeter + próprias submissões + chat), anônimo (ler publicados + submeter em public_form + chat via token).
...
- **Checklist obrigatório antes de fechar iteração** (bloqueante):
  ...
  9.  Toda role (super_admin, owner, editor, viewer, autenticado não-membro, anônimo) que interage com a entidade afetada foi verificada nesta iteração — não apenas a role mencionada no escopo original.
  10. Toda superfície pública (/api/public/*, páginas públicas, /explore) que expõe a entidade afetada foi revisada quanto a compatibilidade.
  11. Se um novo tipo de campo/tabela/papel foi introduzido, confirmar que os componentes genéricos (DynamicGrid, DynamicForm, resolução de computed) o suportam — não apenas a tela específica da iteração.

Se qualquer item do checklist falhar, a iteração **não fecha** — corrigir antes de avançar. 

**Auditoria de coerência a cada 3 iterações:** revisar o documento de estado atual inteiro contra a skill/plano, procurando especificamente por: (a) funcionalidades descritas como "fora de escopo" que já foram implementadas sem atualização da seção; (b) tipos/campos/papéis novos que não foram propagados aos componentes genéricos; (c) qualquer tela ou endpoint que ainda usa a versão antiga de uma entidade que mudou de formato.